...
Fuel efficiency, as reflected in the MPG rating for a vehicle is heavily influenced by weight of the vehicle, qsec (the 1/4 mile time for the vehicle), and transmission type.

Manual transmissions do significantly affect the gas mileage of a vehicle, and reflect an increase of roughly `r round(fitmultamwtqsec$coef[2],2)` miles per gallon over automatic transmissions when other variables are accounted for.  However a manual transmission also increases the impact of weight on gas mileage.

Code and figures are in the appendix.
...
We are most likely overfitting hugely here.  If we take the three variables with the lowest p values, wt, am, and qsec, and fit them we get 

```{r, FitAmWtQsecCoefficients, results='markup'}
summary(fitamwtqsec)$coefficients
```

This looks pretty good, and it matches what we get if we run step() on the original fit.  However we should note that although qsec does not have much correlation with weight or transmission type, weight and transmission type have a correlation of `r round(cor(mtcars$wt, mtcars$am),3)`. (see information on stepped fit in the appendix)

If we introduce an interaction term for weight and transmission type, we get 
...
```{r, DataSetup, echo=TRUE, comment=FALSE, message=FALSE, eval=FALSE}
```

### Miscellaneous Code

P-values for fit on all data:

```{r, FitAllPValues,  echo=TRUE, comment=FALSE, message=FALSE, eval=FALSE}
```

Coefficients for fit on am + wt + qsec:

```{r, FitAmWtQsecCoefficients,  echo=TRUE, comment=FALSE, message=FALSE, eval=FALSE}
```

Coefficients for final fit on am * wt + qsec:

```{r, FinalFitCoefficients,  echo=TRUE, comment=FALSE, message=FALSE, eval=FALSE}
```

Comparison of $R^2$ values:

```{r, R2Comparison,  echo=TRUE, comment=FALSE, message=FALSE, eval=FALSE}
```

Confidence interval for am coefficient:

```{r, AMConfInterval,  echo=TRUE, comment=FALSE, message=FALSE, eval=FALSE}
```

Confidence interval for am:wt coefficient:

```{r, AMWTConfInterval,   echo=TRUE, comment=FALSE, message=FALSE, eval=FALSE}
```
...
```{r, PairsPlot, echo=TRUE, comment=FALSE, message=FALSE }
...
Running step() against the full data set: 

```{r, Step, echo=TRUE, message=FALSE, comment=FALSE, eval=FALSE}
```

```{r, echo=TRUE, results="markup"}
summary(steppedfit)$coefficients
```
